...
interface UseChatbotReturn {
    ...
    clearHistory: () => void;
    updateContext: (context: ChatContextData) => void;
}
...
export const useChatbot = (): UseChatbotReturn => {
   ...
   * Skips API call when no disease context — uses instant local greeting
   */
    const initializeChat = useCallback(async (context?: ChatContextData) => {
        ...
        // Only call greeting API if we have actual disease context
        const hasContext = context && context.disease && context.disease !== 'None' && context.crop !== 'General';
        ...
        if (hasContext) {
            try {
                ...
                console.error('[Chatbot] Greeting fetch failed:', err);
            }
        }

        // Instant local greeting — no API call needed
        setMessages([{
            text: "Hello! I'm your AgriDetect AI assistant. Ask me anything about farming, crop diseases, or treatment. 🌱",
            isUser: false,
            timestamp: new Date(),
        }]);
    }, [i18n.language]);
   ...
   */
    ...
    /**
     * Update chat context without reinitializing (keeps messages)
     */
    const updateContext = useCallback((newContext: ChatContextData) => {
