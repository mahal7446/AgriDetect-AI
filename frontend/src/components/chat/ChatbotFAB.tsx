...
export const ChatbotFAB = () => {
  ...
  // Chatbot hook
  const { messages, isLoading, sendMessage, initializeChat, clearHistory, updateContext } = useChatbot();

  // Track previous context to detect disease changes
  ...
  // Stable context key to detect when disease changes
  const contextKey = useMemo(
    () => context ? `${context.crop}:${context.disease}` : null,
    [context]
  );

  // Initialize chat when opened, OR reset when disease context changes
  useEffect(() => {
    if (!isOpen) return;

    const prevKey = prevContextRef.current;
    const hasMessages = messages.length > 0;

    // First open with no messages — initialize
    if (!hasMessages) {
    ...
    // Context changed (user scanned a different disease) — reset chat
    if (contextKey && prevKey && contextKey !== prevKey) {
      console.log('[ChatbotFAB] Disease context changed, resetting chat');
      prevContextRef.current = contextKey;
      clearHistory();
      // clearHistory sets messages to [] which will trigger this effect again
      // and hit the !hasMessages branch above
      return;
    }

    // Context appeared for the first time while chat was open
    if (contextKey && !prevKey) {
      prevContextRef.current = contextKey;
      updateContext(context!);
    }
  }, [isOpen, messages.length, contextKey, context, initializeChat, clearHistory, updateContext]);
